...
var Data = function (_React$Component) {
  ...
  function Data(props, context) {
    ...
    _this.onNext = function (data) {
      if (data) {
        if (!(0, _lodash2.default)(data, _this.state.data)) {
    ...
    _this.fetchData = function (overrides) {
      ...
      var fetchCount = _this.state.fetchCount + 1;

      // Creates a callback which only completes if fetchData hasn't been called
      // again in the meantime
      var makeCallback = function makeCallback(fn) {
        return function () {
          if (_this.state.fetchCount === fetchCount) fn.apply(undefined, arguments);
        };
      };
      var actionWithVariables = function actionWithVariables() {
        return action(variables, _this.context);
      };

      _this.setState({ loading: true, fetchCount: fetchCount });

      store.observeData(null, actionWithVariables, makeCallback(_this.onNext), makeCallback(_this.onError), _extends({}, options, overrides)).then(function (_ref) {
